...
Certification ran a full volume suite after backend restart (9.0 minutes wall clock).  
Core product paths (local analysis, forecast, cache warm path, sessions, concurrent users) are strong.  
**Production is blocked primarily by internet-retrieval worker stalls** (single uvicorn worker becomes unresponsive under hard open-world topics) and **memory continuity regressions** on multi-turn follow-ups.
